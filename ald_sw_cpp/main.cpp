...
cv::Mat fg_obj_segmentation(cv::Mat &img, cv::Mat &bgModel);
cv::Mat med_filtr_morph_op(cv::Mat fgMask, cv::Mat kernel);
cv::Mat mov_obj_segmentation(cv::Mat &img);
cv::Mat bg_mdl_actualization(cv::Mat &inputImage, cv::Mat &bgModel,cv::Mat &fgMask, cv::Mat &movMask);
void calc_area_and_ratio_indicators(std::list<recog_object> &act_list, std::list<recog_object> &prev_list);
void check_for_dang_obj(std::list<recog_object> &prev_list, cv::Mat &imageToShow);
...
int main() {
    ...
    for(int iImage = iStart; iImage < iEnd; iImage += iStep) {
        ...
        fgMask = fg_obj_segmentation(inputImage, bgModel);

        // Median filtration of foreground mask, morphological operations
        cv::Mat kernel = cv::getStructuringElement(cv::MORPH_RECT,  cv::Size(ksize, ksize));
        fgMask = med_filtr_morph_op(fgMask, kernel);

        // Moving objects segmentation
        movMask = mov_obj_segmentation(inputImage);

        // Median filtration of moving objects mask, morphological operations
        movMask = med_filtr_morph_op(movMask, kernel);

        // Background model actualization
        bgModel = bg_mdl_actualization(inputImage, bgModel,fgMask, movMask);
        ...
        // Add new objects to actual list of objects
        cv::Rect minRect;
...
//            cv::putText(imageToShow, "obj", cv::Point(int(mc[i].x), int(mc[i].y)), cv::FONT_HERSHEY_SIMPLEX, 0.2, color);
        ...
        // Calculate area and ratio indicators
        calc_area_and_ratio_indicators(act_list, prev_list);

        // Check for dangerous objects
        check_for_dang_obj(prev_list, imageToShow);
...
cv::Mat fg_obj_segmentation(cv::Mat &img, cv::Mat &bgModel)
{
    cv::Mat fgMask = cv::Mat::zeros(img.rows, img.cols, CV_8U);
    int sum = 0;
    for(int jj=0; jj < img.rows; jj++) {
        for (int ii = 0; ii < img.cols; ii++) {
            vec_uchar_3 in_pixel = img.at<vec_uchar_3>(jj, ii);
...
cv::Mat med_filtr_morph_op(cv::Mat mask, cv::Mat kernel)
{
    //cv::imshow("Przed", fgMask);
    cv::medianBlur(mask, mask, median_ksize);
...
cv::Mat mov_obj_segmentation(cv::Mat &img)
{
    cv::Mat movMask = cv::Mat::zeros(img.rows, img.cols, CV_8U);
    for(int jj=0; jj < img.rows; jj++) {
        for (int ii = 0; ii < img.cols; ii++) {
            vec_uchar_3 in_pixel = img.at<vec_uchar_3>(jj, ii);
            vec_uchar_3 prev_pixel = img.at<vec_uchar_3>(jj, ii);
            int mov_sum = 0;
            for (int k = 0; k < 3; k++)
